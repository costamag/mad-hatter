...
template<uint32_t NumVars, uint32_t CutSize, typename Func>
...
{
  lut_decomposer<CutSize, NumVars> decomposer;
...
TEST_CASE( "LUT decomposer for parity 4 3- shannon", "[lut_synthesis]" )
{
  ...
  kitty::print_binary( ctt ); std::cout << std::endl;
  itt_l func( ctt );
  ...
  test_lut_dec<num_vars, cut_size, itt_l>( func, times, supps, funcs );
}
